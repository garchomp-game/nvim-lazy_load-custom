...
return {
  {
    ...
    config = function()
      ...
      -- Masonを利用して指定された言語サーバーを自動的にインストールする設定を行う
      ...
      require("mason-lspconfig").setup_handlers {
        ...
        function (server_name)
          -- LSPサーバーの基本的なオプションを設定
          -- ここで設定される `capabilities` は自動補完のための能力を提供する
          local opts = {
            capabilities = capabilities,
          }

          -- カスタム設定が存在する場合は適用
          -- 各サーバーのカスタム設定を適用するために、事前に読み込んだ設定を使用
      ...
      local function check_installation()
        ...
        mr.refresh(function ()
          for _, tool in ipairs(tool_list) do
            local pkg = mr.get_package(tool) -- 各ツールのパッケージ情報を取得
            if pkg and not pkg:is_installed() then
              pkg:install() -- インストールされていないツールをインストール
            end
          end
        end)
      end
